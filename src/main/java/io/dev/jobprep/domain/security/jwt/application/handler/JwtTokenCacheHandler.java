...
@Slf4j
@Service
public class JwtTokenCacheHandler extends AbstractTokenCacheHandler {
    ...
    protected JwtTokenCacheHandler(StringRedisTemplate redisTemplate) {
        super(redisTemplate);
    }
    ...
    protected void verifyCache(String userId, String token) {
        ...
        try {
            ...
            if (!savedToken.equals(token)) {
                log.warn("Refresh token does not match stored token '{}'", token);
                throw new TokenStorageException(ErrorCode400.RFT_VALIDATION_FAILURE);
            }
        } catch (Exception e) {
            throw new TokenStorageException(ErrorCode400.RFT_VALIDATION_FAILURE);
